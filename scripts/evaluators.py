...
def evaluate_value(predicted_df, truth_df, observed_df, target_df, user_df):
    ...
    group1_average_errors = np.array([])
    group2_average_errors = np.array([])
    errors = np.array([])
    for item in experiment_frame.index.unique(level=1):
        try:
            group1_average_error = (group_1_by_item.get_group(item).val_predicted - group_1_by_item.get_group(item).val_truth).mean()
            group2_average_error = (group_2_by_item.get_group(item).val_predicted - group_2_by_item.get_group(item).val_truth).mean()
        except KeyError as ignored:
            continue
        group1_average_errors = np.append(group1_average_errors, group1_average_error)
        group2_average_errors = np.append(group2_average_errors, group2_average_error)
        errors = np.append(errors, (np.abs(group1_average_error - group2_average_error)))
